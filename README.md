...
Once a GPU build of MPAS-Model is available there are two changes which must be made to the workflow
configuration files:
1. Set an attribute in your main scenario yaml file to point to the GPU enabled mpas_atmosphere executable:
Add the following t the yaml file
```
build:
  forecast directory: <path to the folder containing the gpu build of mpas_atmosphere>
```
2. Set the number of GPU's on each node in `scenarios/defaults/forecast.yaml`:
```
job:
    defaults:
        GPUPerNode: 4
